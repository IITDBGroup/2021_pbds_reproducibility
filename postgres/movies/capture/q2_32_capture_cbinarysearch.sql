...
set_bit(0::bit(32), binary_search_array_pos(ARRAY [1,868,1773,2642,3499,4357,5211,6065,6931,8140,25974,27548,34065,46062,53769,60293,68067,72593,78211,81853,86372,89850,93391,97031,100291,103619,106674,109846,113350,116879,120936,126987,131263], movieid) - 1, 1)
AS "PROV_movies1"
...
set_bit(0::bit(32), binary_search_array_pos(ARRAY [1,95,218,329,435,552,745,925,1091,1213,1269,1380,1590,1779,2003,2167,2396,2639,2826,3083,3403,3717,4019,4465,5008,5754,6564,7361,26094,43652,55290,72407,131263], movieid) - 1, 1) AS "PROV_ratings1"
FROM "ratings" AS F0) F1)) F0
WHERE (((F0."movieid" = F0."movieid1") and (F0."genres" LIKE '%Comedy%')) and (F0."ryear" >= 2010))
