SELECT fast_bit_or(F0."PROV_movies1") AS "PROV_movies1", fast_bit_or(F0."PROV_ratings1") AS "PROV_ratings1"
FROM (
...
FROM (
SELECT count(F0."rating") AS "AGGR_0", F0."movieid" AS "GROUP_0", F0."title" AS "GROUP_1", set_bits(F0."PROV_movies1")::bit(64) AS "PROV_movies1", set_bits(F0."PROV_ratings1")::bit(64) AS "PROV_ratings1"
FROM (
