...
FROM "movies" AS F0
WHERE ((F0."movieid" >= 275) AND (F0."movieid" < 413))) F0 CROSS JOIN (
...
FROM "ratings" AS F0
WHERE ((((F0."movieid" >= 277) AND (F0."movieid" < 303)) OR ((F0."movieid" >= 308) AND (F0."movieid" < 327))) OR ((F0."movieid" >= 355) AND (F0."movieid" < 371)))) F1)) F0
WHERE (F0."movieid" = F0."movieid1")
GROUP BY F0."movieid", F0."title") F0) F0
WHERE (F0."num_ratings" >= 63300)) F0;
