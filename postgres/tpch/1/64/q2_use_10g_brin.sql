...
FROM "partsupp" AS F0
WHERE (F0."ps_partkey" <@ '{1,62383,94130,314809,346905,719596,750217,780945,811843,841830,872886,935985,967525,1028728,1090113,1306198,1337439,1526173,1557642,1589457,1653105,1715597,1780431,2000001}'::int[])) F0 CROSS JOIN (
...
FROM "supplier" AS F0
WHERE (F0."s_suppkey" <@ '{1,14063,15625,25000,26563,60937,62500,76562,78125,100001}'::int[])) F1)) F0 CROSS JOIN "nation" AS F1)) F0 CROSS JOIN "region" AS F1)) F0
...
FROM "part" AS F0
WHERE (F0."p_partkey" <@ '{1,63205,94802,315681,348355,441114,472088,720999,752575,783377,813785,845923,877364,939184,969555,1000513,1091908,1123925,1154875,1529987,1560664,1591605,1654958,1719152,1781558,2000001}'::int[])) F0 CROSS JOIN (
SELECT F0."s_suppkey" AS "s_suppkey", F0."s_name" AS "s_name", F0."s_address" AS "s_address", F0."s_nationkey" AS "s_nationkey", F0."s_phone" AS "s_phone", F0."s_acctbal" AS "s_acctbal", F0."s_comment" AS "s_comment"
FROM "supplier" AS F0
...
SELECT F0."ps_partkey" AS "ps_partkey", F0."ps_suppkey" AS "ps_suppkey", F0."ps_availqty" AS "ps_availqty", F0."ps_supplycost" AS "ps_supplycost", F0."ps_comment" AS "ps_comment"
FROM "partsupp" AS F0
WHERE (F0."ps_partkey" <@ '{1,62383,94130,314809,346905,719596,750217,780945,811843,841830,872886,935985,967525,1028728,1090113,1306198,1337439,1526173,1557642,1589457,1653105,1715597,1780431,2000001}'::int[])) F1)) F0 CROSS JOIN "nation" AS F1)) F0 CROSS JOIN "region" AS F1)) F0 CROSS JOIN (
