...
FROM "customer" AS F0
WHERE F0."c_custkey" <@ '{1,797177,842308,1500001}'::int[]) F0 CROSS JOIN (
...
FROM "orders" AS F0
WHERE F0."o_orderkey" <@ '{1,30112293,31919138,60000001}'::int[]) F1)) F0 CROSS JOIN (
...
FROM "lineitem" AS F0
WHERE F0."l_orderkey" <@ '{1,29847456,31642433,60000001}'::int[]) F1)) F0 CROSS JOIN (
...
FROM "lineitem" AS F0
WHERE F0."l_orderkey" <@ '{1,29847456,31642433,60000001}'::int[]
