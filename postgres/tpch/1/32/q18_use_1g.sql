...
FROM "customer" AS F0
WHERE F0."c_custkey" <@ '{14064,18752,32816,37504,65632,70320,117191,121878,126565,131252,140626,150001}'::int[]) F0 CROSS JOIN (
...
FROM "orders" AS F0
WHERE F0."o_orderkey" <@ '{1124994,1312485,1500000,1687491,2062497,2249988,2812485,3187491,3749988,3937479,4687491,4874982}'::int[]) F1)) F0 CROSS JOIN (
...
FROM "lineitem" AS F0
WHERE F0."l_orderkey" <@ '{1125348,1312711,1500738,1687907,2062599,2250438,2813156,3188294,3749957,3937667,4687457,4875014}'::int[]) F1)) F0 CROSS JOIN (
...
FROM "lineitem" AS F0
WHERE F0."l_orderkey" <@ '{1125348,1312711,1500738,1687907,2062599,2250438,2813156,3188294,3749957,3937667,4687457,4875014}'::int[]
