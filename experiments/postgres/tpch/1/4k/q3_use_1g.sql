...
FROM "lineitem" AS F0
WHERE (F0."l_orderkey" <@ '{491013,492480,992771,994242,1188034,1189536,2298756,2300385,2435301,2436960,2454819,2456452,2626945,2628391,3459552,3461094,4876864,4878311,5520453,5521860}'::int[])) F0 CROSS JOIN (
...
FROM "orders" AS F0
WHERE (F0."o_orderkey" <@ '{491648,493091,992257,993732,1187622,1189127,2300005,2301508,2435043,2436547,2455969,2457504,2628103,2629601,3459431,3460932,4876996,4878466,5520384,5521831}'::int[])) F1)) F0 CROSS JOIN (
...
FROM "customer" AS F0
WHERE (F0."c_custkey" <@ '{31650,31688,32775,32813,39150,39188,49613,49650,59588,59625,69938,69975,99150,99187,106687,106725,131362,131400,140662,140700}'::int[])) F1)) F0
