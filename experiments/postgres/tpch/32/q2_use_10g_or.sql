...
FROM "partsupp" AS F0
WHERE ((F0."ps_partkey" >= '1') AND (F0."ps_partkey" < '2000001'))) F0 CROSS JOIN (
...
FROM "supplier" AS F0
WHERE ((F0."s_suppkey" >= '1') AND (F0."s_suppkey" < '100001'))) F1)) F0 CROSS JOIN "nation" AS F1)) F0 CROSS JOIN "region" AS F1)) F0
...
FROM "part" AS F0
WHERE (((F0."p_partkey" >= '1') AND (F0."p_partkey" < '1005240')) OR ((F0."p_partkey" >= '1065464') AND (F0."p_partkey" < '2000001')))) F0 CROSS JOIN (
SELECT F0."s_suppkey" AS "s_suppkey", F0."s_name" AS "s_name", F0."s_address" AS "s_address", F0."s_nationkey" AS "s_nationkey", F0."s_phone" AS "s_phone", F0."s_acctbal" AS "s_acctbal", F0."s_comment" AS "s_comment"
FROM "supplier" AS F0
) F1)) F0 CROSS JOIN (
SELECT F0."ps_partkey" AS "ps_partkey", F0."ps_suppkey" AS "ps_suppkey", F0."ps_availqty" AS "ps_availqty", F0."ps_supplycost" AS "ps_supplycost", F0."ps_comment" AS "ps_comment"
FROM "partsupp" AS F0
WHERE ((F0."ps_partkey" >= '1') AND (F0."ps_partkey" < '2000001'))) F1)) F0 CROSS JOIN "nation" AS F1)) F0 CROSS JOIN "region" AS F1)) F0 CROSS JOIN (
