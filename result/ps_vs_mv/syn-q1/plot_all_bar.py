...
#store file names
fname = ["view_size_1m_selectivity","view_size_01m_selectivity","view_size_10m_selectivity", "view_size_20m_selectivity","view_size_40m_selectivity"]
#fname = ["view_size_40m_selectivity"]
#the column in .csv file (each column one line)
num_range = ["No-PS","PS","MV","PS-on-MV"]
#each line color
#l_color = ["Deepskyblue","Red","Springgreen","Hotpink"]
l_color = ["Red","Deepskyblue","orange","green"]
...
for f in range(len(fname)):
	...
	for i in range(len(num_range)):
		...
		if(f == 0):
			plt.legend(loc='upper left',bbox_to_anchor=(0,0.93),prop={'size': 20}, ncol=2, facecolor='white', framealpha=1)
		elif(f == 1):
			plt.legend(loc='upper left',bbox_to_anchor=(0,0.93),prop={'size': 20}, ncol=2, facecolor='white', framealpha=1)
		elif(f == 2):
