...
#store file names
fname = ["monetdb_tpch_1GB_capture", "monetdb_tpch_10GB_capture"]
#fname = ["post_tpch_1GB_runtime_overview"]
#the column in .csv file (each column one line)
num_range = ["NOR","PS32","PS64","PS400","PS4000","PS10000"]
#each line color
#l_color = ["Deepskyblue","Red","Springgreen","Hotpink"]
#l_color = ["Deepskyblue","Red"]
l_color = ["Red","Deepskyblue","Hotpink","Springgreen","Black","Darkorange"]
...
for f in range(len(fname)):
	...
	for i in range(len(num_range)):
		...
		if(f==0 and i > 0):
			...
			#plt.legend(loc='upper left',prop={'size': 18}, ncol=1)
			plt.legend().remove()
			plt.ylim([0, 6])
			plt.yticks([0,1,2,3,4,5],['0','100%','200%','300%','400%','500%'])
			plt.xticks(x1+width*i-0.3, ('Q2','Q3','Q10','Q15','Q18'))
