...
num_range = ["No-PS","eager"]
#each line color
#l_color = ["Deepskyblue","Red","Springgreen","Hotpink"]
#l_color = ["Deepskyblue","Red","Darkorange"]
...
for f in range(len(fname)):
	...
	for i in range(len(num_range)):
		...
		if(f==0):
			...
			#plt.legend().remove()
			plt.legend(loc='upper center',prop={'size': 25}, ncol=3)
			plt.ylim([0, 4000])
			plt.yticks([500,1000,1500,2000,2500,3000],['500','1,000','1,500','2,000','2,500','3,000'])
			plt.xticks(x1+width*i-0.1, ('5%','10%','20%'))
		elif(f==1):
			plt.bar(x1+width*i, y1, width, alpha=0.7, label=num_range[i],color=l_color[i],bottom=0)
			plt.legend().remove()
			plt.yticks([0,1,2,3])
			...
			plt.bar(x1+width*i, y1, width, alpha=0.5, label=num_range[i],color=l_color[i],bottom=0)
			#plt.yscale("log",basey=10)
			plt.legend(loc='upper left',prop={'size': 25}, ncol=1)
	...
	if(f==0 or f==2):
		...
		plt.xlabel('Selectivity',fontsize=36)
	elif(f==1):
