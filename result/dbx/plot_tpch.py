...
for f in range(len(fname)):
	...
	for i in range(len(num_range)):
		...
		#if(f==0):
		if(i == 0):
			plt.bar(x1+width*i-0.2, y1, width, alpha=0.5, label=num_range[i],color=l_color[i],bottom=0.01)
		else:
			plt.bar(x1+width*i-0.2, y1, width, alpha=0.5, label=num_range[i],color=l_color[i],bottom=0.01,hatch = '/')
		plt.yscale("log",base=10)
		plt.legend(loc='lower right',prop={'size': 26}, ncol=1,facecolor='white', framealpha=1)
		#plt.xticks(x1+width*i-0.2, ('Q3','Q10','Q15','Q18'))
		plt.xticks([0,1,2,3,4,5,6], ['Q2','Q3','Q10','Q18','Q19','Q20','Q21']);
		#plt.ylim([0.1, 100])
	...
	plt.tick_params(axis='y', which='major', labelsize=34)
